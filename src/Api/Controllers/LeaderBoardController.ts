...
export default class LeaderBoardController{
    ...
    async getLeaderBoard(
      req: Request<any, any, GetLeaderBoardRequest>, 
      res: Response
      ): Promise<Response> {
        try {
            const {_id} = req.body;
    ...
    async updateLeaderBoard(
      req: Request<any, any, UpdateLeaderBoardRequest>, 
      res: Response
      ): Promise<Response> {
        try {
            const {_id, isActive, isDelete, userId, xp} = req.body;
            const data: any = {
              _id: _id,
              isActive: isActive,
              isDelete: isDelete,
            ...
            const result: any = await UpdateLeaderBoardHandler(data);
            return res.status(result.statusCode).json({message: result});
        } catch (error: any) {
            return res.status(500).json({error: error.messgae});
        }
    ...
    async deleteLeaderBoard(
      req: Request<any, any, DeleteLeaderBoardRequest>, 
      res: Response
      ): Promise<Response> {
        try {
          const {_id} = req.body;
